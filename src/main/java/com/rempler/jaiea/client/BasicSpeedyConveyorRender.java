...
import com.rempler.jaiea.common.block.BasicSpeedyConveyors;
...
public class BasicSpeedyConveyorRender<T extends BasicSpeedyConveyors> implements IConveyorModelRender<T> {
    private final ResourceLocation active;
    private final ResourceLocation inactive;

    public BasicSpeedyConveyorRender(ResourceLocation active, ResourceLocation inactive)
    {
        this.active = active;
    ...
    protected void addCoverToQuads(List<BakedQuad> baseModel, RenderContext<T> context)
    {
        ...
        Function<Direction, TextureAtlasSprite> getSpriteHorizontal = f -> f.getAxis()== Direction.Axis.Y?null: getSprite.apply(f);
        ...
        Function<Vec3[], Vec3[]> vertexTransformer = conDir== ConveyorHandler.ConveyorDirection.HORIZONTAL? vertices -> vertices: vertices -> {
            ...
            for(int i = 0; i < ret.length; i++)
                ret[i] = new Vec3(vertices[i].x, vertices[i].y+(vertices[i].z==(conDir== ConveyorHandler.ConveyorDirection.UP?0: 1)?1: 0), vertices[i].z);
            return ret;
        };
