 ...
 */
public abstract class BasicVerticalSpeedyConveyors extends BasicSpeedyConveyors
{
    public BasicVerticalSpeedyConveyors(BlockEntity tile)
    {
        super(tile);
    }
    ...
    public void onEntityCollision(@Nonnull Entity entity)
    {
        ...
        if(outputBlocked&&entity.getY() >= getBlockEntity().getBlockPos().getY()+0.25)
        {
            ...
            if(hasBeenHandled)
                vec = new Vec3(vec.x, my*getMove(), vec.z);
            else
                vec = new Vec3(0, my*getMove(), 0);
        }
    ...
    private static final CachedShapesWithTransform<BasicVerticalSpeedyConveyors.CoveredShapeKey, Direction> COVERED_SHAPES =
    ...
    public VoxelShape getCollisionShape()
    {
        ...
        if(!isCovered())
            ...
            return COVERED_SHAPES.get(new BasicVerticalSpeedyConveyors.CoveredShapeKey(
